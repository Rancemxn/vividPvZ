...
void LawnApp::LoadGroup(const char* theGroupName, int theGroupAveMsToLoad)
{
        ...
        int aGroupTime = max((int)aTimer.GetDuration(), 0);
...
void LawnApp::LoadingThreadProc()
{
        ...
        mMusic->MusicInit();
        int aDuration = max((int)aTimer.GetDuration(), 0);
        aTimer.Start();
        ...
        aDuration = max((int)aTimer.GetDuration(), 0);
        aTimer.Start();
...
int LawnApp::GetSeedsAvailable()
{
        ...
        return (SeedType)min(49, (int)aSeedTypeMax);
}
